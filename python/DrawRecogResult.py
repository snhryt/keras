...
def storeImagesLabelsFilenames(txt_filepath, class_num):
  '''
...
def storeMedoidFontName(list_filepath, index):
  '''
  Return the name of center (called as medoid) font in a cluster. 
  '''
  with open(list_filepath) as f:
    ...
    while line:
      ...
      if cluster_index == index:
        medoid_font_name = line.split(' ')[1]
        medoid_font_name = medoid_font_name.split('\n')[0]
        return medoid_font_name
      else:
        line = f.readline()
...
def main():
  ...
  parser.add_argument(
    ...
    type=str,
    default=('/media/snhryt/Data/Research_Master/Syn_AlphabetImages/selected/100fonts_Clustering' 
              + '/SelectedFonts_100class.txt'), 
    help='.txt filepath of a list of medoid fonts'
  )
  parser.add_argument(
    '--char_imgs_dirpath', 
    type=str, 
    default='/media/snhryt/Data/Research_Master/Syn_AlphabetImages/font', 
  ...
  if isImage(args.path):
    ...
    if args.classification_target == 'character':
      if os.path.splitext(args.path)[1] == '.txt':
        test_imgs, test_labels, test_filenames = storeImagesLabelsFilenames(args.path, 26)
  ...
  if args.classification_target == 'character':
    ...
    log_filename = '{0} of {1} images are correctly classified'.format(corretct_num, len(test_imgs))
    ...
    for img, filename, probs in zip(test_imgs, test_filenames, results):
      img = img.reshape(img.shape[0], img.shape[1])
      medoid_font_name = storeMedoidFontName(args.medoid_fonts_list_filepath, label)
      medoid_img = storeMedoidImage(args.char_imgs_dirpath, 'A', medoid_font_name)
      output_img_filepath = os.path.join(output_dirpath, filename)
