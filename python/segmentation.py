...
class Segmentation:
  resized_height = resized_width = 100
  ...
  def storeTextRegionsRect(self, rect, diff_thresh=10):
    x1, y1, width1, height1 = rect
    area1 = width1 * height1
    for i, text_region_rect in enumerate(self.text_region_rects):
      x2, y2, width2, height2 = text_region_rect
      if x1 <= x2 and y1 <= y2 and width1 >= width2 and height1 >= height2:
        self.text_region_rects[i] = rect
        return
      else:
        x_diff = abs(x1 - x2)
        y_diff = abs(y1 - y2)
        ...
        if ( (x_diff <= diff_thresh and width_diff <= diff_thresh) 
           or (y_diff <= diff_thresh and height_diff <= diff_thresh) ):
  ...
  def detectTextRegions(self, height_thresh=30):
    ...
    # Store 5 channel(RGB + Lightness + Gradient magnitude) images
    ...
    for channel_img in channel_imgs:
      ...
      for rect in rects:
        if rect[3] < height_thresh:
          continue
        self.storeTextRegionsRect(rect)
#         if len(self.text_region_rects) == 0:
#           self.text_region_rects.append(rect)
#         else:
#           same_region_index = self.getSameTextRegionIndex(rect)
#           if same_region_index >= 0:
#             if rect[2] >= self.text_region_rects[same_region_index][2]:
...
#   #     if isBlackBackground(bin_img):
#   #       bin_img = 255 - bin_img
#       for rect in self.text_region_rects:
#         x, y, width, height = rect
#         x_min = x - 1 if (x > 0) else x
#         y_min = y - 1 if (y > 0) else y
#         x_max = x_min + width + 1 if (x_min + width <= self.width) else x_min + width
#         y_max = y_min + height + 1 if (y_min + height <= self.height) else y_min + height
#         rect[0] = x_min
#         rect[1] = y_min
#         rect[2] = x_max - x_min
#         rect[3] = y_max - y_min
#         text_region_img = bin_img[y_min:y_max+1, x_min:x_max+1]
#         if not isNoisy(text_region_img):
#           self.text_region_imgs.append(text_region_img)
...
#       labeled_img = cv2.connectedComponents(masked_img)[1]
      ...
      for label in range(1, labeled_img.max() + 1):
        ...
        resized_img = cv2.resize(square_img, (Segmentation.resized_width, 
                                              Segmentation.resized_height))
...
#         cutting_x.append(index)
#       cutting_x.append(white_col_x[index])
#       while index < len(white_col_x) - 1:
#         if white_col_x[index] - 1 == white_col_x[index - 1] and white_col_x[index] + 1 == white_col_x[index + 1]:
#           index += 1
#         else:
#           cutting_x.append(white_col_x[index])
#           index += 1
...
#       for j in range(0, len(cutting_x) - 1):
#         segmented_img = img[0:height, cutting_x[j]:cutting_x[j + 1]]
#         if self.isGoodCharImage(segmented_img):
#           square_img = getSquareImage(segmented_img)
#           resized_img = cv2.resize(square_img, (Segmentation.resized_width, Segmentation.resized_height))
