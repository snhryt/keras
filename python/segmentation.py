...
def isBlackBackground(bin_img, judge_thresh=0.3):
...
def getSquareImage(bin_img):
    ...
    if height == width:
        ...
        if half_length % 2 == 0:
            ...
            square_img[0:height, half_length + 1:(half_length + 1) + width] = (bin_img[0:height,
                                                                                       0:width])
        return square_img
    else:
        ...
        if half_length % 2 == 0:
            ...
            square_img[half_length + 1:(half_length + 1) + height, 0:width] = (bin_img[0:height, 
                                                                                       0:width])
        return square_img
...
class Segmentation:
    def __init__(self, img, classifier1_filepath='trained_classifierNM1.xml', 
                 classifier2_filepath='trained_classifierNM2.xml', resized_width=100,
                 resized_height=100):
        self.img = img
        self.classifier1_filepath = classifier1_filepath
        self.classifier2_filepath = classifier2_filepath
    ...
    def segmentTextRegions(self, height_thresh=30):
        ...
        for channel_img in channel_imgs:
            ...
            erfilter1 = cv2.text.createERFilterNM1(
                cv2.text.loadClassifierNM1(self.classifier1_filepath), 40, 0.00015, 0.13, 0.2, True,
                0.1
            )
            erfilter2 = cv2.text.createERFilterNM2(
                cv2.text.loadClassifierNM2(self.classifier2_filepath), 0.5
            )
            ...
            for rect in rects:
                ...
                if ( height < height_thresh or isBlackBackground(text_region_img) 
                     or isNoisy(text_region_img) ):
                    continue
            ...
            for i, img in enumerate(self.text_region_imgs):
                ...
                for j in range(len(cutting_x) - 1):
                    ...
                    if self.isGoodCharImage(segmented_img):
                        square_img = getSquareImage(segmented_img)
                        resized_img = cv2.resize(square_img, 
