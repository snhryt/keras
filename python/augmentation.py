...
def embedImage(fimg, x_tl=0, y_tl=0, angle=0.0):
    ...
    if y_start >= fimg_height or x_start >= fimg_width:
        raise ValueError(
            'x_tl and y_tl must be in (' 
            + '{0}, {1})'.format(-((bimg_width - fimg_width) / 2), (bimg_width - fimg_width) / 2)
        )
    ...
    if height == width:
        ...
        if height > width:
            ...
            if half_length % 2 == 0:
                ...
                square_img[0:height, half_length + 1:(half_length + 1) + width] = (roi_img[0:height, 
                                                                                           0:width])
        else:
            ...
            if half_length % 2 == 0:
                ...
                square_img[half_length + 1:(half_length + 1) + height, 0:width] = (roi_img[0:height, 
                                                                                           0:width])
        return square_img
...
def storeScaleDownImages(imgs):
    ...
    for i,img in enumerate(imgs):
        ...
        for delta in [6, 12, 18, 24, 30]:
            ...
            bin_resized_img = cv2.threshold(resized_img, 0, 255, 
