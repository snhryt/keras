...
class Training:
    ...
    def buildLenet(self):
        ...
        model.add(
             Convolution2D(filters=20, kernel_size=(8, 8), strides=(3, 3), 
                           input_shape=(self.height, self.width, self.channel_num), name='conv1')
        )
        model.add(
        ...
        model.add(
             Convolution2D(filters=50, kernel_size=(5, 5), strides=(1, 1), name='conv2')
        )
        model.add(
             MaxPooling2D(pool_size=(2, 2), strides=(2, 2))
        )
    ...
    def buildCaffenet(self):
        ...
        # conv2
        model.add(
             Convolution2D(filters=256, kernel_size=(4, 4), border_mode='same', 
                           W_regularizer=l2(weight_decay), name='conv2')
        )
        model.add(Activation('relu'))
        model.add(
             MaxPooling2D(pool_size=(3, 3), strides=(2, 2))
        ...
        # conv3
        model.add(
             Convolution2D(filters=384, kernel_size=(3, 3), border_mode='same', 
                           W_regularizer=l2(weight_decay), name='conv3')
        )
        model.add(Activation('relu'))
        # conv4
        model.add(
             Convolution2D(filters=384, kernel_size=(3, 3), border_mode='same', 
                           W_regularizer=l2(weight_decay), name='conv4')
        )
        model.add(Activation('relu'))
        # conv5
        model.add(
             Convolution2D(filters=256, kernel_size=(3, 3), border_mode='same', 
                           W_regularizer=l2(weight_decay), name='conv5')
        )
        model.add(Activation('relu'))
        model.add(
             MaxPooling2D(pool_size=(3, 3), strides=(2, 2))
        ...
        # fc6
        model.add(
             Dense(4096, W_regularizer=l2(weight_decay), name='fc6')
        )
        model.add(Activation('relu'))
        model.add(Dropout(0.5))
        # fc7
        model.add(
             Dense(4096, W_regularizer=l2(weight_decay), name='fc7')
        )
        model.add(Activation('relu'))
        model.add(Dropout(0.5))
        # fc8
        model.add(
             Dense(self.class_num, W_regularizer=l2(weight_decay), name='fc8')
        )
        model.add(Activation('softmax'))
        model.summary()
        return model
    ...
    def train(self):
        ...
        if self.need_generator:
            ...
            if self.need_augmentation:
                train_imgs, train_labels = self.storeAugmentedBinImages(train_filepaths, 
                                                                        train_labels)
            else:
